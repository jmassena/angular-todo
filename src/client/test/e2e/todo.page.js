...
TodoPage.prototype.divMainContentClick = function (browser) {
  browser.actions().mouseMove(this.divMainContent, {
    x: 0,
    y: 0
  }).click().perform();
};
...
TodoCells.prototype = Object.create({}, {
  ...
  row: {
    get: function () {
      return this.parent.todoList.get(this.idx);
    }
  },
  done: {
    get: function () {
      return this.row.all(by.model('todo.done')).first();
    }
  },
  title: {
    get: function () {
      return this.row.all(by.css('.todo-title')).first().getText();
    }
  },
  notes: {
    get: function () {
      return this.row.all(by.css('.todo-notes')).first().getText();
    }
  },
  dueDateTime: {
    get: function () {
      return this.row.all(by.css('.todo-duedate')).first().getText();
    }
  },
  dueDateTitle: {
    get: function () {
      return this.row.all(by.css('.todo-duedate'))
        .first().getAttribute('title');
    }
  },
  edit: {
    get: function () {
      return this.row.all(by.css('a.action-edit')).first();
    }
  },
  delete: {
    get: function () {
      return this.row.all(by.css('a.action-delete')).first();
    }
  },
  deleteYes: {
    get: function () {
      return this.row.all(by.css('button.action-delete-yes')).first();
    }
  },
  deleteNo: {
    get: function () {
      return this.row.all(by.css('button.action-delete-no')).first();
    }
  },
  deleteModal: {
    get: function () {
      return this.row.all(by.css('ul.delete-modal')).first();
    }
  }
});
